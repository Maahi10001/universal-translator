...
public class MessageParserFactory {
    public static MessageParser getParser(String messageContent) {
        ...
        if (trimmed.startsWith("<?xml") || messageContent.contains("pacs")) {
            return new MXParser(); // Handles pacs.008, pacs.009, camt.053, etc.
        } else {
//            if (trimmed.startsWith("{1:") || trimmed.matches("(?s).*\\{\\d:.*"))
            return new MTParser(); // Handles MT103, MT202, etc.
        }
